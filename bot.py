...
# you configure the bot by editing .env instead of exporting shell variables.
load_dotenv()

# Load environment variables
# SECURITY: the token must ONLY come from the environment. Never hardcode a
# real bot token in source code — if this file is ever shared, uploaded, or
# pushed to a repo, a hardcoded token is instantly compromised.
DISCORD_TOKEN = os.getenv('DISCORD_TOKEN')
BOT_NAME = os.getenv('BOT_NAME', 'StoneNodes')
PREFIX = os.getenv('PREFIX', '+')
YOUR_SERVER_IP = os.getenv('YOUR_SERVER_IP', '127.0.0.1')
MAIN_ADMIN_ID = int(os.getenv('MAIN_ADMIN_ID', '0'))
VPS_USER_ROLE_ID = int(os.getenv('VPS_USER_ROLE_ID', '0'))
DEFAULT_STORAGE_POOL = os.getenv('DEFAULT_STORAGE_POOL', 'default')
BOT_VERSION = os.getenv('BOT_VERSION', '2.1')
BOT_DEVELOPER = os.getenv('BOT_DEVELOPER', 'StoneNodes Team')
# Logo/icon shown on every embed. Replace with your own hosted image URL.
BOT_LOGO_URL = os.getenv('BOT_LOGO_URL', 'https://i.imgur.com/2yYjGwT.png')

if not DISCORD_TOKEN:
    raise SystemExit(
        "DISCORD_TOKEN is not set. Put it in your .env file or export it as "
        "an environment variable before starting the bot. Never paste your "
        "token directly into this file."
    )

# ---------------------------------------------------------------------------
# Theme: Green & Stone
# ---------------------------------------------------------------------------
COLOR_STONE = 0x3F4B3D      # neutral / general embeds (dark stone-green)
COLOR_GREEN = 0x3CB878      # success / primary accent
COLOR_ERROR = 0xA13636      # errors (muted stone-red)
COLOR_INFO = 0x4F7965       # info (deep stone-green)
COLOR_WARNING = 0xB98D45    # warnings (stone amber)
...
def create_success_embed(title, description=""):
    return create_embed(title, description, color=COLOR_GREEN)
...
class ManageView(discord.ui.View):
    def __init__(self, user_id, vps_list, is_shared=False, owner_id=None, is_admin=False, actual_index: Optional[int] = None):
        ...
        if len(vps_list) > 1:
            ...
            self.add_item(self.select)
            self.initial_embed = create_embed("VPS Management", "Select a VPS from the dropdown menu below.", COLOR_STONE)
    ...
    async def create_vps_embed(self, index):
        ...
        status_color = COLOR_GREEN if status == 'running' and not suspended else COLOR_WARNING if suspended else COLOR_ERROR
    ...
    async def action_callback(self, interaction: discord.Interaction, action: str):
        ...
        if self.selected_index is None:
            ...
            return
        actual_idx = self.actual_index if self.actual_index is not None else self.indices[self.selected_index]
        ...
        if action == 'start':
            ...
            try:
                ...
                if ssh_url:
                    try:
                        ssh_embed = create_embed("🔑 SSH Access", f"SSH connection for VPS `{container_name}`:", COLOR_GREEN)
...
async def manage_vps(ctx, arg1: str = None, arg2: str = None):
    """
    Usage:
      {PREFIX}manage                -> browse all your VPS
      {PREFIX}manage <vps-id>       -> jump straight to that VPS's control panel
      {PREFIX}manage @user          -> (admin) browse a user's VPS
      {PREFIX}manage @user <vps-id> -> (admin) jump straight to a user's VPS
    """
    async def try_member(value):
        if value is None:
            return None
        try:
    ...
    for raw in (arg1, arg2):
        if raw is None:
            continue
        member_match = await try_member(raw)
        if member_match:
            target_member = member_match
        elif raw.isdigit():
            vps_number = int(raw)
        else:
            await ctx.send(embed=create_error_embed(
                "Invalid Argument",
                f"`{raw}` isn't a valid VPS ID or @mention. Usage: `{PREFIX}manage [vps-id] [@user]`"
            ))
            return
    ...
    if target_member:
        ...
        vps_list = vps_data.get(user_id, [])
        if not vps_list:
            await ctx.send(embed=create_error_embed("No VPS Found", f"{target_member.mention} doesn't have any {BOT_NAME} VPS."))
            return
        if vps_number is not None:
            if vps_number < 1 or vps_number > len(vps_list):
                await ctx.send(embed=create_error_embed("Invalid VPS ID", f"{target_member.mention} only has {len(vps_list)} VPS. Valid IDs: 1-{len(vps_list)}."))
                return
            view = ManageView(str(ctx.author.id), [vps_list[vps_number - 1]], is_admin=True, owner_id=user_id, actual_index=vps_number - 1)
            embed = await view.get_initial_embed()
            await ctx.send(embed=embed, view=view)
        else:
            view = ManageView(str(ctx.author.id), vps_list, is_admin=True, owner_id=user_id)
            await ctx.send(embed=create_info_embed(f"Managing {target_member.name}'s VPS", f"Managing VPS for {target_member.mention}"), view=view)
        return
    ...
    if vps_number is not None:
        if vps_number < 1 or vps_number > len(vps_list):
            await ctx.send(embed=create_error_embed("Invalid VPS ID", f"You only have {len(vps_list)} VPS. Valid IDs: 1-{len(vps_list)}. Use `{PREFIX}manage` to see them all."))
            return
        view = ManageView(user_id, [vps_list[vps_number - 1]], actual_index=vps_number - 1)
        embed = await view.get_initial_embed()
        await ctx.send(embed=embed, view=view)
        return
...
async def vps_list(ctx, node_id: int = 1):
    ...
    color = COLOR_GREEN if is_online else COLOR_ERROR  # Teal green / Soft red for modern feel
    embed = create_embed(
...
async def list_all_vps(ctx):
    ...
    if vps_info:
        ...
        for idx, chunk in enumerate(chunks, 1):
            embed = create_embed(f"VPS Details (Part {idx})", "List of all VPS deployments", COLOR_STONE)
...
async def share_user(ctx, shared_user: discord.Member, vps_number: int):
    ...
    try:
        await shared_user.send(embed=create_embed("VPS Access Granted", f"You have access to VPS #{vps_number} from {ctx.author.mention}. Use `{PREFIX}manage-shared {ctx.author.mention} {vps_number}`", COLOR_GREEN))
    except discord.Forbidden:
...
async def revoke_share(ctx, shared_user: discord.Member, vps_number: int):
    ...
    try:
        await shared_user.send(embed=create_embed("VPS Access Revoked", f"Your access to VPS #{vps_number} by {ctx.author.mention} has been revoked.", COLOR_ERROR))
    except discord.Forbidden:
        await ctx.send(embed=create_info_embed("Notification Failed", f"Could not DM {shared_user.mention}"))
...
async def ports_command(ctx, subcmd: str = None, *args):
    ...
    if subcmd == 'add':
        ...
        if success:
            ...
            await ctx.send(embed=create_error_embed("Not Found", f"Forward ID not found. Use {PREFIX}ports list."))
    else:
...
async def admin_add(ctx, user: discord.Member):
    ...
    try:
        await user.send(embed=create_embed("🎉 Admin Role Granted", f"You are now an admin by {ctx.author.mention}", COLOR_GREEN))
    except discord.Forbidden:
...
async def admin_remove(ctx, user: discord.Member):
    ...
    try:
        await user.send(embed=create_embed("⚠️ Admin Role Revoked", f"Your admin role was removed by {ctx.author.mention}", COLOR_ERROR))
    except discord.Forbidden:
        await ctx.send(embed=create_info_embed("Notification Failed", f"Could not DM {user.mention}"))
...
async def admin_list(ctx):
    ...
    embed = create_embed("👑 Admin Team", "Current administrators:", COLOR_STONE)
...
async def vps_info(ctx, container_name: str = None):
    if not container_name:
        ...
        chunks = [vps_text[i:i+1024] for i in range(0, len(vps_text), 1024)]
        for idx, chunk in enumerate(chunks, 1):
            embed = create_embed(f"🖥️ All VPS (Part {idx})", f"List of all VPS deployments", COLOR_STONE)
            add_field(embed, "VPS List", chunk, False)
            await ctx.send(embed=embed)
    else:
        found_vps = None
        ...
        embed = create_embed(f"🖥️ VPS Information - {container_name}", f"Details for VPS owned by {found_user.mention}{suspended_text}{whitelisted_text} on node {node_name}", COLOR_STONE)
...
async def execute_command(ctx, container_name: str, *, command: str):
    ...
    try:
        ...
        embed = create_embed(f"Command Output - {container_name}", f"Command: `{command}`", COLOR_STONE)
        if output.strip():
            if len(output) > 1000:
                output = output[:1000] + "\n... (truncated)"
...
async def resource_monitor_control(ctx, action: str = "status"):
    ...
    if action.lower() == "status":
        ...
        embed = create_embed("Resource Monitor Status", f"Resource monitoring is currently **{status}** (logs only; no auto-stop)", COLOR_INFO if resource_monitor_active else COLOR_WARNING)
...
async def vps_stats(ctx, container_name: str):
    ...
    try:
        stats = await get_container_stats(container_name, node_id)
        embed = create_embed(f"📊 VPS Statistics - {container_name}", f"Resource usage statistics", COLOR_STONE)
...
async def vps_network(ctx, container_name: str, action: str, value: str = None):
    ...
    try:
        if action.lower() == "list":
            ...
            if len(output) > 1000:
                output = output[:1000] + "\n... (truncated)"
            embed = create_embed(f"🌐 Network Interfaces - {container_name}", "Network configuration", COLOR_STONE)
...
async def vps_processes(ctx, container_name: str):
    ...
    try:
        ...
        if len(output) > 1000:
            output = output[:1000] + "\n... (truncated)"
        embed = create_embed(f"⚙️ Processes - {container_name}", "Running processes", COLOR_STONE)
...
async def vps_logs(ctx, container_name: str, lines: int = 50):
    ...
    try:
        ...
        if len(output) > 1000:
            output = output[:1000] + "\n... (truncated)"
        embed = create_embed(f"📋 Logs - {container_name}", f"Last {lines} log lines", COLOR_STONE)
...
async def quick_help(ctx):
    ...
    add_field(embed, "👤 For Users", 
        ...
        f"• `{PREFIX}manage` or `{PREFIX}manage <vps-id>` - Start/stop/manage VPS\n"
    ...
    embed.set_footer(text=f"{BOT_NAME} VPS Manager • Use {PREFIX}help for complete command list")
    await ctx.send(embed=embed)
...
async def help_search(ctx, *, search_term: str = None):
    ...
    embed.set_footer(text=f"{BOT_NAME} VPS Manager • Use {PREFIX}help for complete list")
    await ctx.send(embed=embed)    
...
class HelpView(discord.ui.View):
        ...
        self.command_categories = {
            "user": {
                ...
                "commands": [
                    ...
                    (f"{PREFIX}myvps", "List your VPS"),
                    (f"{PREFIX}manage [vps-id] [@user]", "Manage your VPS — add a VPS ID to jump straight to it. Admins can add @user to manage theirs"),
    ...
    def update_embed(self):
        ...
        colors = {
            "user": 0x8FBC8F,       # Sage green
            "vps": 0x2ecc71,        # Green
            "ports": 0x6E8B74,      # Stone green
            "system": 0x708060,     # Mossy stone
            "bot": 0x3E8E5A,        # Deep green
            "nodes": 0x7C9885,      # Sage stone
            "admin": 0xA9BA9D,      # Light stone green
            "main_admin": 0xC2B280  # Khaki stone
        }
        color = colors.get(self.current_category, COLOR_STONE)
